// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from cobot1_interfaces:action/Assembly.idl
// generated code does not contain a copyright notice
#include "cobot1_interfaces/action/detail/assembly__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `tasks`
#include "cobot1_interfaces/msg/detail/block_task__functions.h"

bool
cobot1_interfaces__action__Assembly_Goal__init(cobot1_interfaces__action__Assembly_Goal * msg)
{
  if (!msg) {
    return false;
  }
  // tasks
  if (!cobot1_interfaces__msg__BlockTask__Sequence__init(&msg->tasks, 0)) {
    cobot1_interfaces__action__Assembly_Goal__fini(msg);
    return false;
  }
  return true;
}

void
cobot1_interfaces__action__Assembly_Goal__fini(cobot1_interfaces__action__Assembly_Goal * msg)
{
  if (!msg) {
    return;
  }
  // tasks
  cobot1_interfaces__msg__BlockTask__Sequence__fini(&msg->tasks);
}

bool
cobot1_interfaces__action__Assembly_Goal__are_equal(const cobot1_interfaces__action__Assembly_Goal * lhs, const cobot1_interfaces__action__Assembly_Goal * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // tasks
  if (!cobot1_interfaces__msg__BlockTask__Sequence__are_equal(
      &(lhs->tasks), &(rhs->tasks)))
  {
    return false;
  }
  return true;
}

bool
cobot1_interfaces__action__Assembly_Goal__copy(
  const cobot1_interfaces__action__Assembly_Goal * input,
  cobot1_interfaces__action__Assembly_Goal * output)
{
  if (!input || !output) {
    return false;
  }
  // tasks
  if (!cobot1_interfaces__msg__BlockTask__Sequence__copy(
      &(input->tasks), &(output->tasks)))
  {
    return false;
  }
  return true;
}

cobot1_interfaces__action__Assembly_Goal *
cobot1_interfaces__action__Assembly_Goal__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cobot1_interfaces__action__Assembly_Goal * msg = (cobot1_interfaces__action__Assembly_Goal *)allocator.allocate(sizeof(cobot1_interfaces__action__Assembly_Goal), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(cobot1_interfaces__action__Assembly_Goal));
  bool success = cobot1_interfaces__action__Assembly_Goal__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
cobot1_interfaces__action__Assembly_Goal__destroy(cobot1_interfaces__action__Assembly_Goal * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    cobot1_interfaces__action__Assembly_Goal__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
cobot1_interfaces__action__Assembly_Goal__Sequence__init(cobot1_interfaces__action__Assembly_Goal__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cobot1_interfaces__action__Assembly_Goal * data = NULL;

  if (size) {
    data = (cobot1_interfaces__action__Assembly_Goal *)allocator.zero_allocate(size, sizeof(cobot1_interfaces__action__Assembly_Goal), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = cobot1_interfaces__action__Assembly_Goal__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        cobot1_interfaces__action__Assembly_Goal__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
cobot1_interfaces__action__Assembly_Goal__Sequence__fini(cobot1_interfaces__action__Assembly_Goal__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      cobot1_interfaces__action__Assembly_Goal__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

cobot1_interfaces__action__Assembly_Goal__Sequence *
cobot1_interfaces__action__Assembly_Goal__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cobot1_interfaces__action__Assembly_Goal__Sequence * array = (cobot1_interfaces__action__Assembly_Goal__Sequence *)allocator.allocate(sizeof(cobot1_interfaces__action__Assembly_Goal__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = cobot1_interfaces__action__Assembly_Goal__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
cobot1_interfaces__action__Assembly_Goal__Sequence__destroy(cobot1_interfaces__action__Assembly_Goal__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    cobot1_interfaces__action__Assembly_Goal__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
cobot1_interfaces__action__Assembly_Goal__Sequence__are_equal(const cobot1_interfaces__action__Assembly_Goal__Sequence * lhs, const cobot1_interfaces__action__Assembly_Goal__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!cobot1_interfaces__action__Assembly_Goal__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
cobot1_interfaces__action__Assembly_Goal__Sequence__copy(
  const cobot1_interfaces__action__Assembly_Goal__Sequence * input,
  cobot1_interfaces__action__Assembly_Goal__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(cobot1_interfaces__action__Assembly_Goal);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    cobot1_interfaces__action__Assembly_Goal * data =
      (cobot1_interfaces__action__Assembly_Goal *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!cobot1_interfaces__action__Assembly_Goal__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          cobot1_interfaces__action__Assembly_Goal__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!cobot1_interfaces__action__Assembly_Goal__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `error_message`
#include "rosidl_runtime_c/string_functions.h"

bool
cobot1_interfaces__action__Assembly_Result__init(cobot1_interfaces__action__Assembly_Result * msg)
{
  if (!msg) {
    return false;
  }
  // failed_step
  // error_message
  if (!rosidl_runtime_c__String__init(&msg->error_message)) {
    cobot1_interfaces__action__Assembly_Result__fini(msg);
    return false;
  }
  return true;
}

void
cobot1_interfaces__action__Assembly_Result__fini(cobot1_interfaces__action__Assembly_Result * msg)
{
  if (!msg) {
    return;
  }
  // failed_step
  // error_message
  rosidl_runtime_c__String__fini(&msg->error_message);
}

bool
cobot1_interfaces__action__Assembly_Result__are_equal(const cobot1_interfaces__action__Assembly_Result * lhs, const cobot1_interfaces__action__Assembly_Result * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // failed_step
  if (lhs->failed_step != rhs->failed_step) {
    return false;
  }
  // error_message
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->error_message), &(rhs->error_message)))
  {
    return false;
  }
  return true;
}

bool
cobot1_interfaces__action__Assembly_Result__copy(
  const cobot1_interfaces__action__Assembly_Result * input,
  cobot1_interfaces__action__Assembly_Result * output)
{
  if (!input || !output) {
    return false;
  }
  // failed_step
  output->failed_step = input->failed_step;
  // error_message
  if (!rosidl_runtime_c__String__copy(
      &(input->error_message), &(output->error_message)))
  {
    return false;
  }
  return true;
}

cobot1_interfaces__action__Assembly_Result *
cobot1_interfaces__action__Assembly_Result__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cobot1_interfaces__action__Assembly_Result * msg = (cobot1_interfaces__action__Assembly_Result *)allocator.allocate(sizeof(cobot1_interfaces__action__Assembly_Result), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(cobot1_interfaces__action__Assembly_Result));
  bool success = cobot1_interfaces__action__Assembly_Result__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
cobot1_interfaces__action__Assembly_Result__destroy(cobot1_interfaces__action__Assembly_Result * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    cobot1_interfaces__action__Assembly_Result__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
cobot1_interfaces__action__Assembly_Result__Sequence__init(cobot1_interfaces__action__Assembly_Result__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cobot1_interfaces__action__Assembly_Result * data = NULL;

  if (size) {
    data = (cobot1_interfaces__action__Assembly_Result *)allocator.zero_allocate(size, sizeof(cobot1_interfaces__action__Assembly_Result), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = cobot1_interfaces__action__Assembly_Result__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        cobot1_interfaces__action__Assembly_Result__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
cobot1_interfaces__action__Assembly_Result__Sequence__fini(cobot1_interfaces__action__Assembly_Result__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      cobot1_interfaces__action__Assembly_Result__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

cobot1_interfaces__action__Assembly_Result__Sequence *
cobot1_interfaces__action__Assembly_Result__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cobot1_interfaces__action__Assembly_Result__Sequence * array = (cobot1_interfaces__action__Assembly_Result__Sequence *)allocator.allocate(sizeof(cobot1_interfaces__action__Assembly_Result__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = cobot1_interfaces__action__Assembly_Result__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
cobot1_interfaces__action__Assembly_Result__Sequence__destroy(cobot1_interfaces__action__Assembly_Result__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    cobot1_interfaces__action__Assembly_Result__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
cobot1_interfaces__action__Assembly_Result__Sequence__are_equal(const cobot1_interfaces__action__Assembly_Result__Sequence * lhs, const cobot1_interfaces__action__Assembly_Result__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!cobot1_interfaces__action__Assembly_Result__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
cobot1_interfaces__action__Assembly_Result__Sequence__copy(
  const cobot1_interfaces__action__Assembly_Result__Sequence * input,
  cobot1_interfaces__action__Assembly_Result__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(cobot1_interfaces__action__Assembly_Result);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    cobot1_interfaces__action__Assembly_Result * data =
      (cobot1_interfaces__action__Assembly_Result *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!cobot1_interfaces__action__Assembly_Result__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          cobot1_interfaces__action__Assembly_Result__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!cobot1_interfaces__action__Assembly_Result__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


bool
cobot1_interfaces__action__Assembly_Feedback__init(cobot1_interfaces__action__Assembly_Feedback * msg)
{
  if (!msg) {
    return false;
  }
  // current_index
  return true;
}

void
cobot1_interfaces__action__Assembly_Feedback__fini(cobot1_interfaces__action__Assembly_Feedback * msg)
{
  if (!msg) {
    return;
  }
  // current_index
}

bool
cobot1_interfaces__action__Assembly_Feedback__are_equal(const cobot1_interfaces__action__Assembly_Feedback * lhs, const cobot1_interfaces__action__Assembly_Feedback * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // current_index
  if (lhs->current_index != rhs->current_index) {
    return false;
  }
  return true;
}

bool
cobot1_interfaces__action__Assembly_Feedback__copy(
  const cobot1_interfaces__action__Assembly_Feedback * input,
  cobot1_interfaces__action__Assembly_Feedback * output)
{
  if (!input || !output) {
    return false;
  }
  // current_index
  output->current_index = input->current_index;
  return true;
}

cobot1_interfaces__action__Assembly_Feedback *
cobot1_interfaces__action__Assembly_Feedback__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cobot1_interfaces__action__Assembly_Feedback * msg = (cobot1_interfaces__action__Assembly_Feedback *)allocator.allocate(sizeof(cobot1_interfaces__action__Assembly_Feedback), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(cobot1_interfaces__action__Assembly_Feedback));
  bool success = cobot1_interfaces__action__Assembly_Feedback__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
cobot1_interfaces__action__Assembly_Feedback__destroy(cobot1_interfaces__action__Assembly_Feedback * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    cobot1_interfaces__action__Assembly_Feedback__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
cobot1_interfaces__action__Assembly_Feedback__Sequence__init(cobot1_interfaces__action__Assembly_Feedback__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cobot1_interfaces__action__Assembly_Feedback * data = NULL;

  if (size) {
    data = (cobot1_interfaces__action__Assembly_Feedback *)allocator.zero_allocate(size, sizeof(cobot1_interfaces__action__Assembly_Feedback), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = cobot1_interfaces__action__Assembly_Feedback__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        cobot1_interfaces__action__Assembly_Feedback__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
cobot1_interfaces__action__Assembly_Feedback__Sequence__fini(cobot1_interfaces__action__Assembly_Feedback__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      cobot1_interfaces__action__Assembly_Feedback__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

cobot1_interfaces__action__Assembly_Feedback__Sequence *
cobot1_interfaces__action__Assembly_Feedback__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cobot1_interfaces__action__Assembly_Feedback__Sequence * array = (cobot1_interfaces__action__Assembly_Feedback__Sequence *)allocator.allocate(sizeof(cobot1_interfaces__action__Assembly_Feedback__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = cobot1_interfaces__action__Assembly_Feedback__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
cobot1_interfaces__action__Assembly_Feedback__Sequence__destroy(cobot1_interfaces__action__Assembly_Feedback__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    cobot1_interfaces__action__Assembly_Feedback__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
cobot1_interfaces__action__Assembly_Feedback__Sequence__are_equal(const cobot1_interfaces__action__Assembly_Feedback__Sequence * lhs, const cobot1_interfaces__action__Assembly_Feedback__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!cobot1_interfaces__action__Assembly_Feedback__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
cobot1_interfaces__action__Assembly_Feedback__Sequence__copy(
  const cobot1_interfaces__action__Assembly_Feedback__Sequence * input,
  cobot1_interfaces__action__Assembly_Feedback__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(cobot1_interfaces__action__Assembly_Feedback);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    cobot1_interfaces__action__Assembly_Feedback * data =
      (cobot1_interfaces__action__Assembly_Feedback *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!cobot1_interfaces__action__Assembly_Feedback__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          cobot1_interfaces__action__Assembly_Feedback__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!cobot1_interfaces__action__Assembly_Feedback__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
#include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `goal`
// already included above
// #include "cobot1_interfaces/action/detail/assembly__functions.h"

bool
cobot1_interfaces__action__Assembly_SendGoal_Request__init(cobot1_interfaces__action__Assembly_SendGoal_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    cobot1_interfaces__action__Assembly_SendGoal_Request__fini(msg);
    return false;
  }
  // goal
  if (!cobot1_interfaces__action__Assembly_Goal__init(&msg->goal)) {
    cobot1_interfaces__action__Assembly_SendGoal_Request__fini(msg);
    return false;
  }
  return true;
}

void
cobot1_interfaces__action__Assembly_SendGoal_Request__fini(cobot1_interfaces__action__Assembly_SendGoal_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // goal
  cobot1_interfaces__action__Assembly_Goal__fini(&msg->goal);
}

bool
cobot1_interfaces__action__Assembly_SendGoal_Request__are_equal(const cobot1_interfaces__action__Assembly_SendGoal_Request * lhs, const cobot1_interfaces__action__Assembly_SendGoal_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // goal
  if (!cobot1_interfaces__action__Assembly_Goal__are_equal(
      &(lhs->goal), &(rhs->goal)))
  {
    return false;
  }
  return true;
}

bool
cobot1_interfaces__action__Assembly_SendGoal_Request__copy(
  const cobot1_interfaces__action__Assembly_SendGoal_Request * input,
  cobot1_interfaces__action__Assembly_SendGoal_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // goal
  if (!cobot1_interfaces__action__Assembly_Goal__copy(
      &(input->goal), &(output->goal)))
  {
    return false;
  }
  return true;
}

cobot1_interfaces__action__Assembly_SendGoal_Request *
cobot1_interfaces__action__Assembly_SendGoal_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cobot1_interfaces__action__Assembly_SendGoal_Request * msg = (cobot1_interfaces__action__Assembly_SendGoal_Request *)allocator.allocate(sizeof(cobot1_interfaces__action__Assembly_SendGoal_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(cobot1_interfaces__action__Assembly_SendGoal_Request));
  bool success = cobot1_interfaces__action__Assembly_SendGoal_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
cobot1_interfaces__action__Assembly_SendGoal_Request__destroy(cobot1_interfaces__action__Assembly_SendGoal_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    cobot1_interfaces__action__Assembly_SendGoal_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
cobot1_interfaces__action__Assembly_SendGoal_Request__Sequence__init(cobot1_interfaces__action__Assembly_SendGoal_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cobot1_interfaces__action__Assembly_SendGoal_Request * data = NULL;

  if (size) {
    data = (cobot1_interfaces__action__Assembly_SendGoal_Request *)allocator.zero_allocate(size, sizeof(cobot1_interfaces__action__Assembly_SendGoal_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = cobot1_interfaces__action__Assembly_SendGoal_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        cobot1_interfaces__action__Assembly_SendGoal_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
cobot1_interfaces__action__Assembly_SendGoal_Request__Sequence__fini(cobot1_interfaces__action__Assembly_SendGoal_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      cobot1_interfaces__action__Assembly_SendGoal_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

cobot1_interfaces__action__Assembly_SendGoal_Request__Sequence *
cobot1_interfaces__action__Assembly_SendGoal_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cobot1_interfaces__action__Assembly_SendGoal_Request__Sequence * array = (cobot1_interfaces__action__Assembly_SendGoal_Request__Sequence *)allocator.allocate(sizeof(cobot1_interfaces__action__Assembly_SendGoal_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = cobot1_interfaces__action__Assembly_SendGoal_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
cobot1_interfaces__action__Assembly_SendGoal_Request__Sequence__destroy(cobot1_interfaces__action__Assembly_SendGoal_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    cobot1_interfaces__action__Assembly_SendGoal_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
cobot1_interfaces__action__Assembly_SendGoal_Request__Sequence__are_equal(const cobot1_interfaces__action__Assembly_SendGoal_Request__Sequence * lhs, const cobot1_interfaces__action__Assembly_SendGoal_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!cobot1_interfaces__action__Assembly_SendGoal_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
cobot1_interfaces__action__Assembly_SendGoal_Request__Sequence__copy(
  const cobot1_interfaces__action__Assembly_SendGoal_Request__Sequence * input,
  cobot1_interfaces__action__Assembly_SendGoal_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(cobot1_interfaces__action__Assembly_SendGoal_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    cobot1_interfaces__action__Assembly_SendGoal_Request * data =
      (cobot1_interfaces__action__Assembly_SendGoal_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!cobot1_interfaces__action__Assembly_SendGoal_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          cobot1_interfaces__action__Assembly_SendGoal_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!cobot1_interfaces__action__Assembly_SendGoal_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `stamp`
#include "builtin_interfaces/msg/detail/time__functions.h"

bool
cobot1_interfaces__action__Assembly_SendGoal_Response__init(cobot1_interfaces__action__Assembly_SendGoal_Response * msg)
{
  if (!msg) {
    return false;
  }
  // accepted
  // stamp
  if (!builtin_interfaces__msg__Time__init(&msg->stamp)) {
    cobot1_interfaces__action__Assembly_SendGoal_Response__fini(msg);
    return false;
  }
  return true;
}

void
cobot1_interfaces__action__Assembly_SendGoal_Response__fini(cobot1_interfaces__action__Assembly_SendGoal_Response * msg)
{
  if (!msg) {
    return;
  }
  // accepted
  // stamp
  builtin_interfaces__msg__Time__fini(&msg->stamp);
}

bool
cobot1_interfaces__action__Assembly_SendGoal_Response__are_equal(const cobot1_interfaces__action__Assembly_SendGoal_Response * lhs, const cobot1_interfaces__action__Assembly_SendGoal_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // accepted
  if (lhs->accepted != rhs->accepted) {
    return false;
  }
  // stamp
  if (!builtin_interfaces__msg__Time__are_equal(
      &(lhs->stamp), &(rhs->stamp)))
  {
    return false;
  }
  return true;
}

bool
cobot1_interfaces__action__Assembly_SendGoal_Response__copy(
  const cobot1_interfaces__action__Assembly_SendGoal_Response * input,
  cobot1_interfaces__action__Assembly_SendGoal_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // accepted
  output->accepted = input->accepted;
  // stamp
  if (!builtin_interfaces__msg__Time__copy(
      &(input->stamp), &(output->stamp)))
  {
    return false;
  }
  return true;
}

cobot1_interfaces__action__Assembly_SendGoal_Response *
cobot1_interfaces__action__Assembly_SendGoal_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cobot1_interfaces__action__Assembly_SendGoal_Response * msg = (cobot1_interfaces__action__Assembly_SendGoal_Response *)allocator.allocate(sizeof(cobot1_interfaces__action__Assembly_SendGoal_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(cobot1_interfaces__action__Assembly_SendGoal_Response));
  bool success = cobot1_interfaces__action__Assembly_SendGoal_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
cobot1_interfaces__action__Assembly_SendGoal_Response__destroy(cobot1_interfaces__action__Assembly_SendGoal_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    cobot1_interfaces__action__Assembly_SendGoal_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
cobot1_interfaces__action__Assembly_SendGoal_Response__Sequence__init(cobot1_interfaces__action__Assembly_SendGoal_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cobot1_interfaces__action__Assembly_SendGoal_Response * data = NULL;

  if (size) {
    data = (cobot1_interfaces__action__Assembly_SendGoal_Response *)allocator.zero_allocate(size, sizeof(cobot1_interfaces__action__Assembly_SendGoal_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = cobot1_interfaces__action__Assembly_SendGoal_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        cobot1_interfaces__action__Assembly_SendGoal_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
cobot1_interfaces__action__Assembly_SendGoal_Response__Sequence__fini(cobot1_interfaces__action__Assembly_SendGoal_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      cobot1_interfaces__action__Assembly_SendGoal_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

cobot1_interfaces__action__Assembly_SendGoal_Response__Sequence *
cobot1_interfaces__action__Assembly_SendGoal_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cobot1_interfaces__action__Assembly_SendGoal_Response__Sequence * array = (cobot1_interfaces__action__Assembly_SendGoal_Response__Sequence *)allocator.allocate(sizeof(cobot1_interfaces__action__Assembly_SendGoal_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = cobot1_interfaces__action__Assembly_SendGoal_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
cobot1_interfaces__action__Assembly_SendGoal_Response__Sequence__destroy(cobot1_interfaces__action__Assembly_SendGoal_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    cobot1_interfaces__action__Assembly_SendGoal_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
cobot1_interfaces__action__Assembly_SendGoal_Response__Sequence__are_equal(const cobot1_interfaces__action__Assembly_SendGoal_Response__Sequence * lhs, const cobot1_interfaces__action__Assembly_SendGoal_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!cobot1_interfaces__action__Assembly_SendGoal_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
cobot1_interfaces__action__Assembly_SendGoal_Response__Sequence__copy(
  const cobot1_interfaces__action__Assembly_SendGoal_Response__Sequence * input,
  cobot1_interfaces__action__Assembly_SendGoal_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(cobot1_interfaces__action__Assembly_SendGoal_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    cobot1_interfaces__action__Assembly_SendGoal_Response * data =
      (cobot1_interfaces__action__Assembly_SendGoal_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!cobot1_interfaces__action__Assembly_SendGoal_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          cobot1_interfaces__action__Assembly_SendGoal_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!cobot1_interfaces__action__Assembly_SendGoal_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"

bool
cobot1_interfaces__action__Assembly_GetResult_Request__init(cobot1_interfaces__action__Assembly_GetResult_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    cobot1_interfaces__action__Assembly_GetResult_Request__fini(msg);
    return false;
  }
  return true;
}

void
cobot1_interfaces__action__Assembly_GetResult_Request__fini(cobot1_interfaces__action__Assembly_GetResult_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
}

bool
cobot1_interfaces__action__Assembly_GetResult_Request__are_equal(const cobot1_interfaces__action__Assembly_GetResult_Request * lhs, const cobot1_interfaces__action__Assembly_GetResult_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  return true;
}

bool
cobot1_interfaces__action__Assembly_GetResult_Request__copy(
  const cobot1_interfaces__action__Assembly_GetResult_Request * input,
  cobot1_interfaces__action__Assembly_GetResult_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  return true;
}

cobot1_interfaces__action__Assembly_GetResult_Request *
cobot1_interfaces__action__Assembly_GetResult_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cobot1_interfaces__action__Assembly_GetResult_Request * msg = (cobot1_interfaces__action__Assembly_GetResult_Request *)allocator.allocate(sizeof(cobot1_interfaces__action__Assembly_GetResult_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(cobot1_interfaces__action__Assembly_GetResult_Request));
  bool success = cobot1_interfaces__action__Assembly_GetResult_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
cobot1_interfaces__action__Assembly_GetResult_Request__destroy(cobot1_interfaces__action__Assembly_GetResult_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    cobot1_interfaces__action__Assembly_GetResult_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
cobot1_interfaces__action__Assembly_GetResult_Request__Sequence__init(cobot1_interfaces__action__Assembly_GetResult_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cobot1_interfaces__action__Assembly_GetResult_Request * data = NULL;

  if (size) {
    data = (cobot1_interfaces__action__Assembly_GetResult_Request *)allocator.zero_allocate(size, sizeof(cobot1_interfaces__action__Assembly_GetResult_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = cobot1_interfaces__action__Assembly_GetResult_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        cobot1_interfaces__action__Assembly_GetResult_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
cobot1_interfaces__action__Assembly_GetResult_Request__Sequence__fini(cobot1_interfaces__action__Assembly_GetResult_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      cobot1_interfaces__action__Assembly_GetResult_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

cobot1_interfaces__action__Assembly_GetResult_Request__Sequence *
cobot1_interfaces__action__Assembly_GetResult_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cobot1_interfaces__action__Assembly_GetResult_Request__Sequence * array = (cobot1_interfaces__action__Assembly_GetResult_Request__Sequence *)allocator.allocate(sizeof(cobot1_interfaces__action__Assembly_GetResult_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = cobot1_interfaces__action__Assembly_GetResult_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
cobot1_interfaces__action__Assembly_GetResult_Request__Sequence__destroy(cobot1_interfaces__action__Assembly_GetResult_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    cobot1_interfaces__action__Assembly_GetResult_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
cobot1_interfaces__action__Assembly_GetResult_Request__Sequence__are_equal(const cobot1_interfaces__action__Assembly_GetResult_Request__Sequence * lhs, const cobot1_interfaces__action__Assembly_GetResult_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!cobot1_interfaces__action__Assembly_GetResult_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
cobot1_interfaces__action__Assembly_GetResult_Request__Sequence__copy(
  const cobot1_interfaces__action__Assembly_GetResult_Request__Sequence * input,
  cobot1_interfaces__action__Assembly_GetResult_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(cobot1_interfaces__action__Assembly_GetResult_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    cobot1_interfaces__action__Assembly_GetResult_Request * data =
      (cobot1_interfaces__action__Assembly_GetResult_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!cobot1_interfaces__action__Assembly_GetResult_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          cobot1_interfaces__action__Assembly_GetResult_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!cobot1_interfaces__action__Assembly_GetResult_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `result`
// already included above
// #include "cobot1_interfaces/action/detail/assembly__functions.h"

bool
cobot1_interfaces__action__Assembly_GetResult_Response__init(cobot1_interfaces__action__Assembly_GetResult_Response * msg)
{
  if (!msg) {
    return false;
  }
  // status
  // result
  if (!cobot1_interfaces__action__Assembly_Result__init(&msg->result)) {
    cobot1_interfaces__action__Assembly_GetResult_Response__fini(msg);
    return false;
  }
  return true;
}

void
cobot1_interfaces__action__Assembly_GetResult_Response__fini(cobot1_interfaces__action__Assembly_GetResult_Response * msg)
{
  if (!msg) {
    return;
  }
  // status
  // result
  cobot1_interfaces__action__Assembly_Result__fini(&msg->result);
}

bool
cobot1_interfaces__action__Assembly_GetResult_Response__are_equal(const cobot1_interfaces__action__Assembly_GetResult_Response * lhs, const cobot1_interfaces__action__Assembly_GetResult_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // status
  if (lhs->status != rhs->status) {
    return false;
  }
  // result
  if (!cobot1_interfaces__action__Assembly_Result__are_equal(
      &(lhs->result), &(rhs->result)))
  {
    return false;
  }
  return true;
}

bool
cobot1_interfaces__action__Assembly_GetResult_Response__copy(
  const cobot1_interfaces__action__Assembly_GetResult_Response * input,
  cobot1_interfaces__action__Assembly_GetResult_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // status
  output->status = input->status;
  // result
  if (!cobot1_interfaces__action__Assembly_Result__copy(
      &(input->result), &(output->result)))
  {
    return false;
  }
  return true;
}

cobot1_interfaces__action__Assembly_GetResult_Response *
cobot1_interfaces__action__Assembly_GetResult_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cobot1_interfaces__action__Assembly_GetResult_Response * msg = (cobot1_interfaces__action__Assembly_GetResult_Response *)allocator.allocate(sizeof(cobot1_interfaces__action__Assembly_GetResult_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(cobot1_interfaces__action__Assembly_GetResult_Response));
  bool success = cobot1_interfaces__action__Assembly_GetResult_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
cobot1_interfaces__action__Assembly_GetResult_Response__destroy(cobot1_interfaces__action__Assembly_GetResult_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    cobot1_interfaces__action__Assembly_GetResult_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
cobot1_interfaces__action__Assembly_GetResult_Response__Sequence__init(cobot1_interfaces__action__Assembly_GetResult_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cobot1_interfaces__action__Assembly_GetResult_Response * data = NULL;

  if (size) {
    data = (cobot1_interfaces__action__Assembly_GetResult_Response *)allocator.zero_allocate(size, sizeof(cobot1_interfaces__action__Assembly_GetResult_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = cobot1_interfaces__action__Assembly_GetResult_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        cobot1_interfaces__action__Assembly_GetResult_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
cobot1_interfaces__action__Assembly_GetResult_Response__Sequence__fini(cobot1_interfaces__action__Assembly_GetResult_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      cobot1_interfaces__action__Assembly_GetResult_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

cobot1_interfaces__action__Assembly_GetResult_Response__Sequence *
cobot1_interfaces__action__Assembly_GetResult_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cobot1_interfaces__action__Assembly_GetResult_Response__Sequence * array = (cobot1_interfaces__action__Assembly_GetResult_Response__Sequence *)allocator.allocate(sizeof(cobot1_interfaces__action__Assembly_GetResult_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = cobot1_interfaces__action__Assembly_GetResult_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
cobot1_interfaces__action__Assembly_GetResult_Response__Sequence__destroy(cobot1_interfaces__action__Assembly_GetResult_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    cobot1_interfaces__action__Assembly_GetResult_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
cobot1_interfaces__action__Assembly_GetResult_Response__Sequence__are_equal(const cobot1_interfaces__action__Assembly_GetResult_Response__Sequence * lhs, const cobot1_interfaces__action__Assembly_GetResult_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!cobot1_interfaces__action__Assembly_GetResult_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
cobot1_interfaces__action__Assembly_GetResult_Response__Sequence__copy(
  const cobot1_interfaces__action__Assembly_GetResult_Response__Sequence * input,
  cobot1_interfaces__action__Assembly_GetResult_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(cobot1_interfaces__action__Assembly_GetResult_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    cobot1_interfaces__action__Assembly_GetResult_Response * data =
      (cobot1_interfaces__action__Assembly_GetResult_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!cobot1_interfaces__action__Assembly_GetResult_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          cobot1_interfaces__action__Assembly_GetResult_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!cobot1_interfaces__action__Assembly_GetResult_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `feedback`
// already included above
// #include "cobot1_interfaces/action/detail/assembly__functions.h"

bool
cobot1_interfaces__action__Assembly_FeedbackMessage__init(cobot1_interfaces__action__Assembly_FeedbackMessage * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    cobot1_interfaces__action__Assembly_FeedbackMessage__fini(msg);
    return false;
  }
  // feedback
  if (!cobot1_interfaces__action__Assembly_Feedback__init(&msg->feedback)) {
    cobot1_interfaces__action__Assembly_FeedbackMessage__fini(msg);
    return false;
  }
  return true;
}

void
cobot1_interfaces__action__Assembly_FeedbackMessage__fini(cobot1_interfaces__action__Assembly_FeedbackMessage * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // feedback
  cobot1_interfaces__action__Assembly_Feedback__fini(&msg->feedback);
}

bool
cobot1_interfaces__action__Assembly_FeedbackMessage__are_equal(const cobot1_interfaces__action__Assembly_FeedbackMessage * lhs, const cobot1_interfaces__action__Assembly_FeedbackMessage * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // feedback
  if (!cobot1_interfaces__action__Assembly_Feedback__are_equal(
      &(lhs->feedback), &(rhs->feedback)))
  {
    return false;
  }
  return true;
}

bool
cobot1_interfaces__action__Assembly_FeedbackMessage__copy(
  const cobot1_interfaces__action__Assembly_FeedbackMessage * input,
  cobot1_interfaces__action__Assembly_FeedbackMessage * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // feedback
  if (!cobot1_interfaces__action__Assembly_Feedback__copy(
      &(input->feedback), &(output->feedback)))
  {
    return false;
  }
  return true;
}

cobot1_interfaces__action__Assembly_FeedbackMessage *
cobot1_interfaces__action__Assembly_FeedbackMessage__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cobot1_interfaces__action__Assembly_FeedbackMessage * msg = (cobot1_interfaces__action__Assembly_FeedbackMessage *)allocator.allocate(sizeof(cobot1_interfaces__action__Assembly_FeedbackMessage), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(cobot1_interfaces__action__Assembly_FeedbackMessage));
  bool success = cobot1_interfaces__action__Assembly_FeedbackMessage__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
cobot1_interfaces__action__Assembly_FeedbackMessage__destroy(cobot1_interfaces__action__Assembly_FeedbackMessage * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    cobot1_interfaces__action__Assembly_FeedbackMessage__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
cobot1_interfaces__action__Assembly_FeedbackMessage__Sequence__init(cobot1_interfaces__action__Assembly_FeedbackMessage__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cobot1_interfaces__action__Assembly_FeedbackMessage * data = NULL;

  if (size) {
    data = (cobot1_interfaces__action__Assembly_FeedbackMessage *)allocator.zero_allocate(size, sizeof(cobot1_interfaces__action__Assembly_FeedbackMessage), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = cobot1_interfaces__action__Assembly_FeedbackMessage__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        cobot1_interfaces__action__Assembly_FeedbackMessage__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
cobot1_interfaces__action__Assembly_FeedbackMessage__Sequence__fini(cobot1_interfaces__action__Assembly_FeedbackMessage__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      cobot1_interfaces__action__Assembly_FeedbackMessage__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

cobot1_interfaces__action__Assembly_FeedbackMessage__Sequence *
cobot1_interfaces__action__Assembly_FeedbackMessage__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  cobot1_interfaces__action__Assembly_FeedbackMessage__Sequence * array = (cobot1_interfaces__action__Assembly_FeedbackMessage__Sequence *)allocator.allocate(sizeof(cobot1_interfaces__action__Assembly_FeedbackMessage__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = cobot1_interfaces__action__Assembly_FeedbackMessage__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
cobot1_interfaces__action__Assembly_FeedbackMessage__Sequence__destroy(cobot1_interfaces__action__Assembly_FeedbackMessage__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    cobot1_interfaces__action__Assembly_FeedbackMessage__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
cobot1_interfaces__action__Assembly_FeedbackMessage__Sequence__are_equal(const cobot1_interfaces__action__Assembly_FeedbackMessage__Sequence * lhs, const cobot1_interfaces__action__Assembly_FeedbackMessage__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!cobot1_interfaces__action__Assembly_FeedbackMessage__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
cobot1_interfaces__action__Assembly_FeedbackMessage__Sequence__copy(
  const cobot1_interfaces__action__Assembly_FeedbackMessage__Sequence * input,
  cobot1_interfaces__action__Assembly_FeedbackMessage__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(cobot1_interfaces__action__Assembly_FeedbackMessage);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    cobot1_interfaces__action__Assembly_FeedbackMessage * data =
      (cobot1_interfaces__action__Assembly_FeedbackMessage *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!cobot1_interfaces__action__Assembly_FeedbackMessage__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          cobot1_interfaces__action__Assembly_FeedbackMessage__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!cobot1_interfaces__action__Assembly_FeedbackMessage__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}

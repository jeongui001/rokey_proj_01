...
from cobot1_interfaces.action import ExecuteQueue               # 액션 타입
...
class BridgeNode(Node):
    ...
    def _call_sequence_plan(self, grid_json, grid_size):
        """격자 데이터를 SequencerNode에 보내 조립 순서(Action Queue)를 생성 요청.
    ...
    def _send_action_goal(self, action_queue_json):
        ...
        goal.action_queue_json = action_queue_json  # 실행할 동작 큐 JSON
    ...
    def _on_robot_feedback(self, feedback_msg):
        """로봇 실행 중 피드백 수신 — 블록 하나를 배치할 때마다 호출됨.

        피드백 내용: 현재 스텝, 총 스텝, 동작 종류, 색상, 좌표
        이 정보를 DB에 기록하고 Flask에 전달해서 브라우저에 진행률 표시.
        """
        fb = feedback_msg.feedback
        data = {
            'current_step': fb.current_step,      # 현재 몇 번째 동작인지
            'total_steps': fb.total_steps,         # 전체 동작 수
            'current_action': fb.current_action,   # 'place' 또는 'remove'
            'current_color': fb.current_color,     # 블록 색상
            'current_target': list(fb.current_target),  # [x, y, z] 로봇 좌표
        }
        # DB에 배치 기록 저장
        db.insert_placement(fb.current_step, fb.current_color, list(fb.current_target))
        # Flask(→브라우저)에 진행 상황 전송
